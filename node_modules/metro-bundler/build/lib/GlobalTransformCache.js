...
class KeyResultStore {
  ...
  _processResults(keyResults) {var _this3 = this;return _asyncToGenerator(function* () {
      const resultsByKey = new Map(keyResults.map(function (pair) {return [pair.key, pair.result];}));
...
class URIBasedGlobalTransformCache {
  ...
  /**
     ...
     */
  fetch(props) {var _this4 = this;return _asyncToGenerator(function* () {
      const uri = yield _this4._fetcher.fetch(_this4.keyOf(props));
      if (uri == null) {
        return null;
      }
      return yield _this4._fetchResultFromURI(uri);})();
  }
...
class OptionsHasher {
  ...
  /**
     ...
     */
  hashTransformWorkerOptions(hash, options) {const
  ...
  /**
     ...
     * particular file.
     */
  hashTransformOptions(hash, options) {const

    generateSourceMaps =

    options.generateSourceMaps,dev = options.dev,hot = options.hot,inlineRequires = options.inlineRequires,platform = options.platform,projectRoot = options.projectRoot,unknowns = _objectWithoutProperties(options, ['generateSourceMaps', 'dev', 'hot', 'inlineRequires', 'platform', 'projectRoot']);
    const unknownKeys = Object.keys(unknowns);
    if (unknownKeys.length > 0) {
