...
export function drawDiamondPiece(x,y,modules,pieceProperties,props){
  ...
  return (<G key={'g:'+x+':'+y} x={px+xsize/2} y={py+ysize/2} width={xsize} height={ysize}><Rect key={'r:'+x+':'+y} x={-xsize/2} y={-ysize/2} width={xsize} height={ysize} transform={'rotate(45,'+px+xsize/2+', '+py+ysize/2+')'} fill={props.color} /></G>);
}
